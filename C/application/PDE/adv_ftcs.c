/*
  ...
  Solve linear advection equation with FTCS (Forward-Time Centered-Space) Method.
...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<nt;i++)
      ...
      for(j=1;j<nx+2;j++)
	{
	  ...
	  ux[j]=ux[j]-0.5*nu*(ux[j+1]-ux[j-1]);
	...
	total=delt_x*(ux[j+1]-ux[j])+total;
      ...
      total=total+1.;
      //      printf("time=%16.5lf, total=%16.5l,f difference=%16.5lf [%]\n",t,total,fabs((total0-total)/total0)*100);
